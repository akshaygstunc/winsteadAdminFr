...
function setValue(item: CmsItem, field: CmsField, value: any): CmsItem {
  if (field.key in item || ['title', 'subtitle', 'slug', 'status', 'image', 'description', 'sortOrder'].includes(field.key)) {
...
function renderField(field: CmsField, value: any, onChange: (value: any) => void) {
  switch (field.type) {
    case 'textarea':
      return <TextArea label={field.label} value={value || ''} onChange={onChange} rows={field.key.toLowerCase().includes('body') ? 5 : 4} />;
    case 'select':
      ...
      return (
        <div className="space-y-3">
          ...
          <div>
            ...
            <input className="input" type="file" accept={field.type === 'video' ? 'video/mp4' : 'image/*'} onChange={async (e: ChangeEvent<HTMLInputElement>) => {
          ...
          {value ? (field.type === 'video' ? <video src={value} controls className="h-36 w-full rounded-2xl border border-line object-cover" /> : <img src={value} alt={field.label} className="h-36 w-full rounded-2xl border border-line object-cover" />) : null}
        </div>
      );
    default:
      return <div><TextInput label={field.label} value={value || ''} onChange={onChange} />{field.note ? <p className="mt-2 text-xs text-muted">{field.note}</p> : null}</div>;
  }
}
...
export function EditorCmsPage({ config }: { config: CmsConfig }) {
  ...
  const leftFields = useMemo(() => config.fields.filter((f) => ['title','subtitle','slug','image','description','videoUrl'].includes(f.key) || f.type === 'textarea'), [config]);
  const rightFields = useMemo(() => config.fields.filter((f) => !leftFields.includes(f)), [config, leftFields]);

  const load = async (term = '') => {
    try {
      const rows = await api.get<CmsItem[]>(`/content/${config.entity}${term ? `?search=${encodeURIComponent(term)}` : ''}`);
      setItems(rows);
      if (!editingId && rows[0]) setForm({ ...blankFromConfig(config), ...rows[0], data: { ...blankFromConfig(config).data, ...(rows[0].data || {}) } });
    } catch {
  ...
  const submit = async () => {
    try {
      ...
      if (editingId) await api.patch(`/content/${config.entity}/${editingId}`, payload);
    ...
    <DashboardShell>
      ...
      <div className="space-y-6">
        ...
        <SectionCard
          ...
          action={<div className="flex gap-3"><ActionButton secondary onClick={reset}>Reset</ActionButton><ActionButton onClick={submit}>{editingId ? 'Update' : 'Save'}</ActionButton></div>}
        >
          <div className="grid gap-6 xl:grid-cols-[1.6fr_0.9fr]">
            ...
            <div className="space-y-4 rounded-[28px] border border-line bg-panel/50 p-4">
              ...
              {rightFields.map((field) => (
                <div key={field.key}>{renderField(field, getValue(form, field), (value) => setForm((prev) => setValue(prev, field, value)))}</div>
              ))}
              <FormActions onSubmit={submit} onCancel={reset} submitLabel={editingId ? 'Update Record' : 'Create Record'} />
            </div>
          </div>
        </SectionCard>

        <SectionCard title={`${config.title} Listing`} subtitle="Search, review, edit, and remove saved records.">
          <div className="mb-5 flex flex-wrap gap-3">
            <input className="input w-72" placeholder={config.searchPlaceholder || 'Search'} value={search} onChange={(e) => setSearch(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') load(search); }} />
            <ActionButton secondary onClick={() => load(search)}>Search</ActionButton>
          </div>
          <div className="space-y-3">
            {items.map((item) => (
              <div key={item._id || item.title} className="grid gap-4 rounded-[28px] border border-line bg-panel/60 p-4 xl:grid-cols-[140px_1fr_auto] xl:items-center">
                <div className="h-28 overflow-hidden rounded-[22px] border border-line bg-card">
                  {item.image ? <img src={item.image} alt={item.title} className="h-full w-full object-cover" /> : <div className="h-full w-full bg-gradient-to-br from-violet-500/15 to-gold/10" />}
                </div>
                <div>
                  <div className="flex flex-wrap items-center gap-2">
                    <h3 className="text-base font-semibold text-text">{item.title}</h3>
                    <StatusBadge value={item.status || 'draft'} tone={item.status === 'active' || item.status === 'published' ? 'green' : 'slate'} />
                  </div>
                ...
                <div className="justify-self-end"><InlineActions onEdit={() => edit(item)} onDelete={() => remove(item._id)} /></div>
              </div>
            ))}
            {!items.length ? <div className="rounded-3xl border border-dashed border-line p-8 text-sm text-muted">No records found.</div> : null}
          </div>
        </SectionCard>
      </div>
    </DashboardShell>
