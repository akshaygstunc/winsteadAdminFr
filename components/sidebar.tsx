...
function NavEntry({ item, pathname, depth = 0 }: { item: NavItem; pathname: string; depth?: number }) {
      ...
      <div className="space-y-1">
        <button
          ...
          className={clsx(
            ...
            active ? 'border-gold/50 bg-white/[0.04] text-white' : 'border-transparent text-muted hover:border-line hover:bg-panel/70 hover:text-text',
            depth > 0 && 'ml-3 rounded-xl py-2.5',
          )}
        >
          <span className="flex items-center gap-3">
            <Icon className={clsx('h-4 w-4', active ? 'text-gold' : 'text-muted')} />
            <span className="text-sm font-medium">{item.label}</span>
          </span>
          <ChevronDown className={clsx('h-4 w-4 transition', open && 'rotate-180', active ? 'text-gold' : 'text-muted')} />
        </button>
        {open ? (
          <div className="ml-3 space-y-1 border-l border-line/70 pl-3">
            {item.children.map((child) => (
              <NavEntry key={child.href || child.label} item={child} pathname={pathname} depth={depth + 1} />
            ))}
          </div>
        ) : null}
      </div>
  ...
  return (
    ...
    >
      <Icon className={clsx('h-4 w-4', pathname === item.href || pathname.startsWith((item.href || '') + '/') ? 'text-gold' : 'text-muted group-hover:text-gold')} />
      <span className="text-sm font-medium">{item.label}</span>
    </Link>
  );
}
...
export function Sidebar() {
  ...
  const allSections = useMemo(() => navSections, []);
      ...
      <nav className="flex-1 space-y-6 overflow-y-auto px-4 py-4">
        {allSections.map((section) => (
          <div key={section.title}>
            <p className="mb-2 px-2 text-xs uppercase tracking-[0.24em] text-gold/90">{section.title}</p>
            <div className="space-y-1">
              {section.items.map((item) => (
                <NavEntry key={item.href || item.label} item={item} pathname={pathname} />
              ))}
            </div>
          </div>
        ))}
